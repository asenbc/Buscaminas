...
public class GridHelper : MonoBehaviour
{
    ...
    public static void UncoverAllTheMines()
    {
        ...
        //"c" es un iterador que puedes ponerle el nombre que quieras
        foreach (Cell c in cells)
        {
            //Si esa celda tienen una mina
            if (c.hasMine)
            {
                //Llamar al método que carga la textura de la mina
                //Se pone que coge la posición 0 porque en las texturas de hasMine solo hay una textura
                c.LoadTexture(0);
            }
        }
    }
    ...
    public static bool HasMineAt(int x, int y) //La posición de la celda
    {
        ...
        if (x >= 0 && y >= 0 && x < w && y < h)
        {
            ...
            //De esa celda nos devolerá su booleano, si es true habrá mina, si es false no
            return cell.hasMine;
        }
    ...
    //Método para conar las minas adyacentes a una celda (minas alrededor de una celda dada)
    ...
    public static void FloodFillUncover(int x, int y, bool[,] visited) //Le pasamos una posición X e Y, y tenemos un array de booleanos para saber si una celda ya ha sido visitada antes en una posición X e Y
    {
        ...
        if (x >= 0 && y >= 0 && x < w && y < h)
        {
            //Si ya he pasado por esta celda, el algoritmo de FFU no deber nada
            if (visited[x, y])
            {
            ...
            if (adjacentMines > 0)
            {
                //Salimos del método se se cumple la condición
                return;
            }
